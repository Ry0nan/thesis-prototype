...
Prerequisites:
1. Annotated images in YOLO format (exported from Roboflow).
2. Folder structure:
   dataset/yolo/
...
3. A data config YAML (this script creates one automatically if missing).

After training, best weights are saved to models/yolov8_kamias.pt
...
BATCH_SIZE = 16        # If you hit CUDA out-of-memory, drop to 8 or 4
PATIENCE = 20          # Early stop if val mAP doesn't improve for this many epochs
DEVICE = 0             # GPU 0; use "cpu" if no CUDA

# Single-class detection — just locate Kamias.
# Classification (healthy/defective) happens downstream in ResNet18.
CLASS_NAMES = ['kamias']
...
def main():
    ...
    if not os.path.exists(PRETRAINED_MODEL):
        ...
        print("Place yolov8n.pt in the models/ folder.")
        return
    ...
    # Copy best weights to models/ for main.py to find
    best_weights = "../runs/kamias_yolov8n/weights/best.pt"
